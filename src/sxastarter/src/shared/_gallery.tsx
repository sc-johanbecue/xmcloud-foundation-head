...
import { XpImage } from 'src/types/ProductXPs';
import { Carousel, CarouselSlide, useCarousel } from './_carousel';

interface GalleryProps {
  images: RequiredDeep<XpImage>[] | undefined;
  aspectRatio?: number;
  rootProps?: StackProps;
}

export const Gallery = (props: GalleryProps) => {
  const { images, aspectRatio = 1.333, rootProps } = props;
  ...
  const hasNext = currentSlide < (images?.length ?? 0) - 1;
          ...
          {images?.map((image, i) => (
            <CarouselSlide key={i}>
              <AspectRatio
                ...
                _hover={{ opacity: 1 }}
              >
                <Image src={image.Url} objectFit="cover" alt={image.Url} fallback={<Skeleton />} />
              </AspectRatio>
            </CarouselSlide>
          ))}
        </Carousel>
