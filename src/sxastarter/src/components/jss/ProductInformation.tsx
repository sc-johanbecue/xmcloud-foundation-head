...
export const Default = (props: ProductProps): JSX.Element => {
  ...
  async function productFetcher(): Promise<ComposedProduct | null> {
    const routerPath = router?.query?.requestPath;
    ...
    return await GetComposedProduct(
      productIdByField && productIdByField != '' ? productIdByField : productIdByPath ?? ''
    );
  }
    ...
    <div className={`component product ${props.params.styles}`} id={id ? id : undefined}>
      {isLoading ? (
        <div className="centered space-s">
          ...
          <Center>
            <Image
              src={`${IsEditingHost() ? config.publicUrl : ''}/loader_lkw.gif`}
              alt={'Loading'}
              width={150}
              height={150}
            />
            ...
            <Stack spacing={{ base: '6', lg: '8' }} maxW={{ lg: '2xl' }} justify="center">
              <Stack spacing={{ base: '3', md: '4' }}>
                <Stack spacing="2">
                  ...
                  <HStack align={'column'} spacing={'12'} justify={'flex-start'}>
