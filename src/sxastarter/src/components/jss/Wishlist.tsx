...
import { DictionaryPhrases, Field, useSitecoreContext } from '@sitecore-jss/sitecore-jss-nextjs';
import {
  Box,
  Link,
  Heading,
...
export const Default = (props: WishlistProps): JSX.Element => {
  ...
  useEffect(() => {
    ...
    async function GetDictionaryEntries() {
      const dictionaryService = new DictionaryServiceFactory().create(
        sitecoreContext.site?.name ?? config.sitecoreSiteName
      );
