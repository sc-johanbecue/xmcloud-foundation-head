...
export const Default = (props: PromoProps): JSX.Element => {
  ...
  if (props.fields) {
    return (
      <div className={`component newsGrid ${props.params.styles}`}>
        <div className="container">
          <div className="row">
            {props?.fields?.data?.newsItems?.children?.results?.map((element, key) => {
              const roles = element?.roles?.jsonValue?.map((element) => {
                return element?.fields?.Name?.value;
              });
              if (
                !IsVisibilityDateReached(new Date(element?.visibilityDate?.jsonValue?.value)) ||
                !IsAuthenticated(roles, session != null) ||
                !isAuthorized(roles, session?.user?.Role)
              ) {
                return <></>;
              }
              ...
              if (key >= numberOfNews) {
                ...
                return (
                  <Box className={grid} key={key} paddingBottom={4} px={2}>
                    <Link href={element?.url?.path}>
                      <SharedNewsTeaser
